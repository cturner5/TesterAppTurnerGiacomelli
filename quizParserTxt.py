...
#parse the questions, then select ask user for number/range of questions, then parse the quiz out of the questions, then parse answers, combine into dictionary
#ultimate purpose of this class is to create a data structure called "quiz" of the form {question:{number:answer}}
#when making the answers list, it must be a list of lists. Every list must contain every possible answer to the question.
#i.e., if a question has two possible answers, then the answers for that question must be a list with every possible answer.
#the correctness of the user's submission will be tested just by checking if the user's answer is in the answer list
...
class Quiz:
    ...
    def parseAnswersTxt(self, fileName):
        ...
        #makes a list of dictionaries, wherein each dictionary's key is the question number, and the keys value is the corresponding answer(s)
        #this list iwll then be combined with the question list in parseQuestionsTxT() to create the main dictionary structure described in
